...
app.post('/api/categories', [body('name').trim().notEmpty().isLength({max:50})], handleValidation, (req, res) => {
...
app.post('/api/tags', [body('name').trim().notEmpty().isLength({max:30})], handleValidation, (req, res) => {
  let { name } = req.body;
  const slug = slugify(name, { lower: true, strict: true });
