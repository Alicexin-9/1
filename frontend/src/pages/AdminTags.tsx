...
import { tagService } from '../utils/api'
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing) {
        ...
        await tagService.create(formName.trim())
      }
      setShowModal(false)
      fetchTags()
    } catch (e: any) {
    ...
    <div className="max-w-4xl mx-auto">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1">管理博客文章标签</p>
        </div>
        <button onClick={openCreate} className="flex items-center px-4 py-2 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-lg hover:from-purple-700 hover:to-indigo-700">
          <Check size={18} className="mr-1" />新建标签
        </button>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center text-gray-500">暂无标签</div>
        ) : (
                ...
                <tr key={tag.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end space-x-2">
                      <button onClick={() => openEdit(tag)} className="p-2 text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900 rounded-lg">
                        <Edit2 size={16} />
                      </button>
                      <button onClick={() => setDeleteId(tag.id)} className="p-2 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900 rounded-lg">
                        <Trash2 size={16} />
                      </button>
                    </div>
                  </td>
          ...
          <div className="flex items-center justify-center min-h-screen px-4">
            ...
            <div className="relative bg-white dark:bg-gray-800 rounded-xl shadow-xl p-6 w-full max-w-md">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-xl font-bold text-gray-900 dark:text-white">{editing ? '编辑标签' : '新建标签'}</h3>
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="flex items-center justify-center min-h-screen px-4">
            ...
            <div className="relative bg-white dark:bg-gray-800 rounded-xl shadow-xl p-6 w-full max-w-md">
              ...
              <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">确定要删除这个标签吗？如果该标签被文章使用则无法删除。</p>
              <div className="mt-6 flex justify-end space-x-3">
