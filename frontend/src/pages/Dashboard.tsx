...
import { BookOpen, PlusCircle, Edit2, Trash2, Eye, TrendingUp } from 'lucide-react'
...
export default function Dashboard() {
  ...
  const fetchMyPosts = async () => {
    try {
      const response = await axios.get('/api/posts/me/list')
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            <div className="fixed inset-0 transition-opacity bg-black bg-opacity-75" onClick={() => setShowCreateModal(false)}></div>
            ...
            <div className="inline-block w-full max-w-4xl p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white dark:bg-gray-800 rounded-xl shadow-xl">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-2xl font-bold text-gray-900 dark:text-white">
                  {posts.find(p => typeof showDeleteConfirm === 'number' && p.id === showDeleteConfirm) ? '编辑文章' : '创建新文章'}
                </h3>
                <button
                  onClick={() => {
                    setShowCreateModal(false)
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center">
                  ...
                  />
                  <label className="ml-2 text-sm text-gray-700 dark:text-gray-300">立即发布</label>
                </div>
              </div>
              ...
              <div className="mt-6 flex justify-end space-x-3">
                ...
                >
                  {posts.find(p => p.id === showDeleteConfirm) ? '更新文章' : '发布文章'}
                </button>
              </div>
            </div>
          </div>
